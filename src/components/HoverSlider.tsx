...
>(({ text, index, children, className, ...props }, ref) => {
    ...
    return (
        <span
            className={cn(
                "relative flex flex-wrap gap-x-[0.3em] gap-y-1 origin-bottom cursor-pointer",
                className
            )}
            {...props}
            ref={ref}
                ...
                return (
                    <span key={wordIndex} className="inline-flex whitespace-nowrap">
                        {word.split("").map((char, charIndex) => {
                            const charOverallIndex = previousCharsCount + charIndex;
                            return (
                                <span
                                    key={`${char}-${charOverallIndex}`}
                                    className="relative inline-block overflow-hidden"
                                >
                                    <MotionConfig
                                        transition={{
                                            delay: charOverallIndex * 0.025,
                                            duration: 0.3,
